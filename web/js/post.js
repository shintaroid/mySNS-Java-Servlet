...
$(function() {
	$('#file_photo').on("change", function() {
        if($('#file_photo')[0].files.length > 4) {
        	alert("写真は4枚まで選択可能です。");
        }
   });
});
...
$(function(){
	$(document).on('click','#more-read',function(){
		lastpostid = parseInt(window.sessionStorage.getItem(['lastpostid1']));
		nextid = lastpostid + 3;
		window.sessionStorage.setItem(['lastpostid1'],[nextid]);
		for (var i = lastpostid; i < lastpostid + 3; i++) { // **********
			...
			$('#timeline-panel-extends-favorite-form-' + i).append('<input type="hidden" name="message" value="favorite" >'); // **********
			...
			$('#timeline-panel-extends-comment-box-' + i).append('<form id="timeline-panel-extends-comment-form-' + i + '" method="post" action="post.jsp">'); // **********
			...
			$('#timeline-panel-footer-comment-text-' + i).append('<input type="text" class="form-control" placeholder="コメントする" name="comment" required/>');
...
//			alert("success!");
			$(function(){
				obj.parent().toggleClass("favorite-toggle");
				obj.toggleClass("favorite-toggle");
				var flg = obj.val();
				var count = parseInt(obj.parent().parent().find('.favorite-num').val())
				count += 1;
				flg = "true";
				obj.parent().parent().find('.favorite-num').val(count);
				obj.val(flg);
			});
